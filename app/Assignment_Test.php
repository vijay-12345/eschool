...
class Assignment_Test extends Model
{
    ...
    public function getLibraryAssignmentByFilter($request){
        $total_student= Student::select(DB::raw("COUNT(student.id) as total_student"))
                ->where('student.class_section_id',$request->class_section_id)
        ...
        foreach($assignments as $assignment){
            $total_student_submitted= Assignment_Submittted::select(DB::raw("COUNT(assignment_submittted.id) as total_student_submitted"))
                ->where('assignment_submittted.assignment_id',$assignment['id'])
                ->first()->toArray();
            $assignment['total_students']=$total_student['total_student'];
            $assignment['total_student_submitted']=$total_student_submitted['total_student_submitted'];
    ...
    public function deleteAssignment($request){
           $total_student_submitted= Assignment_Submittted::select(DB::raw("COUNT(assignment_submittted.id) as total_student_submitted"))
                ->where('assignment_submittted.assignment_id',$request['id'])
                ->first();
        if($total_student_submitted['total_student_submitted']<1){
